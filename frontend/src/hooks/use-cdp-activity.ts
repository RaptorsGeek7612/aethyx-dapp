...
/** Every CDPManager event for the connected wallet — the credit facility's own side of "what did
 *  I do and when", parallel to useTransactionHistory for the Gateway's deposit/redeem events. The
 *  two are fetched separately (different contract, different event set) and merged for display,
 *  the same way the dashboard already merges per-asset metrics. */
export function useCdpActivity() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["cdpActivity", account, CDP_MANAGER_ADDRESS],
    enabled,
    ...
    queryFn: async (): Promise<CdpActivityEntry[]> => {
      ...
      const address = CDP_MANAGER_ADDRESS as Address;
