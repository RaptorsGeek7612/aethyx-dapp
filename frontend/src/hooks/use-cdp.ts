 ...
 *   instance; if it also predates AUDIT.md finding 12 (`manager.legacyAbi`), its `collaterals`
 *   shape and field indices differ (no liquidationBonusBps), handled below via
 *   cdpManagerLegacyAbi.
 */
export function useCdpPosition(collateralId: Hex, subject?: Address, manager: CdpManagerRef = CURRENT_MANAGER) {
  ...
  const abi = (manager.legacyAbi ? cdpManagerLegacyAbi : cdpManagerAbi) as Abi;
  ...
  const liquidationBonusBps = manager.legacyAbi ? 0 : Number(collateral?.[3] ?? 0);
  const stabilityFeeBps = Number(collateral?.[manager.legacyAbi ? 3 : 4] ?? 0);
  const active = Boolean(manager.legacyAbi ? collateral?.[6] : collateral?.[7]);
